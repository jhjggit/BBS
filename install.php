...
SQL;
        ...
        if(!file_put_contents('inc/install.lock',':))')){
            exit('文件inc/install.lock创建失败，但是您的系统其实已经安装了，您可以手动建立/inc/install.lock文件!');
        }
        if(!file_put_contents('admin/install.lock',':))')){
            exit('文件admin/install.lock创建失败，但是您的系统其实已经安装了，您可以手动建立admin/install.lock文件!');
        }
